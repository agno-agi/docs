...
1. **Install Dependencies**
   ...
   ```bash
   uv pip install agno openai yfinance maxim-py
   ```
...
- **Debug Mode**: Enable debug mode on the Maxim client to see detailed logging information:

  ```python
  instrument_agno(Maxim({"debug": True}).logger())
  ```
