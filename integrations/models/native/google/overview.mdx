...
File Search enables you to upload documents to managed stores and query them with automatic citation extraction. This is ideal for RAG applications where you want Google to handle document processing and retrieval.
...
# Create store and upload document
store = model.create_file_search_store(display_name="My Documents")
...
agent.print_response("What are the main topics in the document?")
```

See the [File Search guide](/integrations/models/native/google/usage/file-search) for detailed usage and examples.
...
| `file_search_store_names` | `Optional[List[str]]` | `None` | List of File Search store names to use for document search. |
| `file_search_metadata_filter` | `Optional[str]` | `None` | Metadata filter string for filtering documents (e.g., 'department="engineering" AND version=2'). |
