...
description: Read `reasoning_content` from an agent's run response and from the final streaming event, using both `reasoning=True` and a dedicated `reasoning_model`.
---
