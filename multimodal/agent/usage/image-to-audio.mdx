...
description: "Analyze an image with `OpenAIChat(id=\"gpt-4o\")` to write a story, then narrate it with `OpenAIChat(id=\"gpt-audio\")`."
---
...
<Steps>
  ...
  <Step title="Add a sample image">
    Save a `sample.jpg` file in the same directory as the script. The agent reads this file with `Image(filepath=image_path)`.
  </Step>
