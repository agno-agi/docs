...
description: "Analyze a video with Gemini and use ffmpeg to cut the highest-scoring 15-60 second segments into short-form clips."
---

Analyze a video and automatically generate short-form segments for platforms like YouTube Shorts and Instagram Reels.
...
<Steps>
  ...
  <Step title="Provide the input video">
    Save or copy an MP4 file next to `video_to_shorts.py` as `sample_video.mp4`.

    <CodeGroup>
    ```bash Mac/Linux
    cp /path/to/input.mp4 sample_video.mp4
    ```

    ```powershell Windows
    Copy-Item C:\path\to\input.mp4 sample_video.mp4
    ```
    </CodeGroup>
  </Step>
