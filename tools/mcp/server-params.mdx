...
description: Understanding how to configure the server parameters for the MCPTools class
---
...
- `env`: Optional environment variables to pass to the MCP server. When you build the `StdioServerParameters` yourself and pass it as `server_params`, only the variables you provide are sent to the server, so include `PATH` and `HOME` yourself if the server needs them. Agno only merges your `env` with a default set of safe variables (like `PATH` and `HOME`) when you pass `command` directly to `MCPTools`.
e.g.
```python
{
