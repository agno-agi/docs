...
If you prefer, you can also use `MCPTools` as an async context manager for automatic resource cleanup:
...
You can set `refresh_connection` on an `MCPTools` instance to refresh the connection to the MCP server on each run.
...
<Note>
The stdio (standard input/output) transport is the default one in Agno's `MCPTools`.
</Note>
