...
```python cookbook/91_tools/file_tools.py
...
<Warning>
`FileTools()` defaults `base_dir` to the process working directory. Set `base_dir` to a narrow directory that contains only the files the agent needs to access.
</Warning>
...
| `max_file_length` | `int` | `10000000` | Maximum decoded text length, measured in Python characters. Reading fails if the decoded content is longer. |
...
- [Tools](https://github.com/agno-agi/agno/blob/v2.7.4/libs/agno/agno/tools/file.py)
