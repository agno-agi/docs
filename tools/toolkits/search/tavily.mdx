...
| `topic`               | `Optional[Literal['general', 'news', 'finance']]` | `None` | Search category - general, news, or finance. Uses the Tavily default when `None`.           |
...
| `auto_parameters`     | `bool`                         | `False`      | Let Tavily auto-tune search parameters; explicitly set parameters (including the always-sent `search_depth` and `include_answer`) take precedence. |
| `chunks_per_source`   | `Optional[int]`                | `None`       | Number of content chunks per source (1-3). Advanced search only.                            |
| `search_params`       | `Optional[Dict[str, Any]]`     | `None`       | Additional parameters merged into web search requests; overrides the named parameters on key collision. |
