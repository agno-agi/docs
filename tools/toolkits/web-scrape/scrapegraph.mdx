...
The following examples require the `scrapegraph-py` and `openai` libraries.

```shell
uv pip install -U scrapegraph-py openai
```

Get an API key from [scrapegraphai.com](https://scrapegraphai.com) and set the `SGAI_API_KEY` environment variable:
...
```python cookbook/91_tools/scrapegraph_tools.py
