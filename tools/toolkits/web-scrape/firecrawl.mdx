...
```python cookbook/91_tools/firecrawl_tools.py
...
| `crawl_website` | Crawls a website using Firecrawl. Parameters include `url` to specify the URL to crawl. The maximum number of pages crawled is set by the toolkit-level `limit` parameter. The function supports optional formats and returns the crawling results in JSON format. |
...
| `search_web` | Performs a web search using Firecrawl. Parameters include `query` for the search term. The maximum number of results is set by the toolkit-level `limit` parameter. Returns search results in JSON format. |

## Developer Resources

- [Tools](https://github.com/agno-agi/agno/blob/main/libs/agno/agno/tools/firecrawl.py)
