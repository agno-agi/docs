...
The pinned source requires `requests`, `AZURE_OPENAI_API_KEY`, `AZURE_OPENAI_ENDPOINT`, and `AZURE_OPENAI_IMAGE_DEPLOYMENT`. `AZURE_OPENAI_API_VERSION` is optional and defaults to `2023-12-01-preview`. The standard-model path also uses `OPENAI_API_KEY`; the full-Azure path uses `AZURE_OPENAI_DEPLOYMENT`.
