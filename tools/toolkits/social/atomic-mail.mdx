...
**AtomicMailTools** give an agent its own email inbox: register an address on [AtomicMail](https://atomicmail.ai), send plain-text email from it, and read what it receives over JMAP. Registration runs through AtomicMail's autonomous proof-of-work signup — no signup form, no domain setup, and no human verification step.
