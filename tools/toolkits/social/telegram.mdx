...
All methods return a JSON string with `{"status": "success", "message_id": ...}` on success or `{"status": "error", "message": ...}` on failure. `delete_message` returns `{"status": "success", "deleted": true}` instead of a `message_id`.

## Developer Resources

- [Tools](https://github.com/agno-agi/agno/blob/v2.7.4/libs/agno/agno/tools/telegram.py)
