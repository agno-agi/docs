...
description: "Insert knowledge and query it asynchronously with ainsert() and aprint_response() on SurrealDB."
---

## Code

```python async_surreal_db.py
import asyncio
...
<Steps>
  ...
  <Step title="Run SurrealDB">
    ```bash
    docker run -d --rm --pull always -p 8000:8000 surrealdb/surrealdb:latest start --user root --pass root
    ```
  </Step>
