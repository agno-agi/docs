...
<Warning>
Agno v2.7.4 constructs the legacy `api.us-east-1.langdb.ai` host when `base_url` is omitted. LangDB's current API guide uses `api.langdb.ai`. Pass `base_url` as shown above.
</Warning>
