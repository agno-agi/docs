...
| **Run evals**        | Runs eval cases with the `smoke` tag daily.              | `ENABLE_SCHEDULED_EVALS` (off by default)    |
...
As you ship more agents, configuration drifts, env vars rot, and new agents miss imports. The template ships a sixth skill for the recurring sweep:
...
You now have a platform that runs locally and on Railway with JWT auth, persists sessions, memory, and traces in Postgres, supports Postgres-backed knowledge, and includes six coding-agent skills for ongoing development and maintenance.
