...
Add profiles to group your cases into suites. The template uses three profiles: `smoke`, `release`, and `live`. This case uses the `live` profile because its answer depends on the open web.
...
<Steps>
  ...
  <Step title="Run the eval suite">
    ...
    ```bash
    python -m evals --profile smoke    # fast suite
...
The template ships a `run_evals` workflow for scheduled checks. Set `ENABLE_SCHEDULED_EVALS=True` to run the `smoke` profile cases daily. See [scheduling](/features/scheduling) for the cron API.
