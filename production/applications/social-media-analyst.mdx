...
description: "Brand intelligence agent that analyzes X (Twitter) sentiment with engagement metrics and actionable recommendations."
---

A brand intelligence agent that analyzes social media sentiment on X (Twitter), extracting engagement metrics, sentiment trends, and actionable recommendations. Produces executive-ready intelligence reports.

## What It Does

- Retrieves tweets using the X API
- Classifies sentiment (positive, negative, neutral, mixed)
- Analyzes engagement patterns (viral, controversy, influence)
- Identifies themes and key issues
...
```bash
python cookbook/01_showcase/01_agents/social_media_analyst/examples/01_brand_analysis.py
```
...
```bash
python cookbook/01_showcase/01_agents/social_media_analyst/examples/02_competitor_compare.py
```
...
```bash
python cookbook/01_showcase/01_agents/social_media_analyst/examples/03_trending_topic.py
```
