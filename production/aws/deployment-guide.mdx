...
description: "Deploy AgentOS on AWS using ECS Fargate, RDS, and ALB template."
keywords:
  [
    "deploy",
    "AWS",
...
Deploy AgentOS to your AWS account using ECS Fargate, RDS PostgreSQL (pgvector), and an Application Load Balancer.
...
| Category          | Details                                              |
| ----------------- | ---------------------------------------------------- |
| **AWS Services**  | ECR, ECS, RDS, ELB, IAM, Secrets Manager, CloudWatch |
| **Tools**         | AWS CLI, Docker, `uv`                                |
| **Configuration** | HTTPS (required for Control Plane), OpenAI API key   |
...
<Warning>
  Avoid `@`, `#`, `%`, `&`, `"`, `'` in `DB_PASS`. These require URL encoding
  and can cause silent connection failures.
</Warning>
...
<CardGroup cols={3}>
  <Card
    ...
    icon="plug"
    href="/production/aws/go-live/connect"
  >
  ...
  <Card
    ...
    icon="repeat"
    href="/production/aws/manage/manage-monitor"
  >
  ...
  <Card
    ...
    icon="alert"
    href="/production/aws/manage/troubleshooting"
  >
