...
in the same fashion as chatting with an Agent or with a LLM works.
...
You can add a `WorkflowAgent` to your workflow like this:
```python
...
Similar to [workflow history for steps](/concepts/workflows/workflow_with_history), the `WorkflowAgent` has access to the full workflow history runs for that session.
This makes it possible to answer questions about previous results, compare outputs from multiple runs, and understand conversation continuity.
