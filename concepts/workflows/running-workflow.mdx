...
You can also choose not to stream executor events: the events emitted by the agents or teams in your Steps.

You can do it by setting `stream_executor_events=False`.

These Workflow events will still be streamed:
- `WorkflowStarted`
- `WorkflowCompleted`
- `StepStarted`
- `StepCompleted` 

The events emitted by the used agents or teams, like `RunContent` and `TeamRunContent`, will not be streamed.
