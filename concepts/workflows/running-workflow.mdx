...
<Note>
  In the case where you put `stream_events=False` (or not set it at
  all), we only yield `WorkflowStartedEvent`, `WorkflowCompletedEvent` along
  with all the `Agent/Team` events.
</Note>

For even more detailed streaming, you can enable intermediate steps by setting `stream_events=True`. This will provide real-time updates about each step of the workflow.
