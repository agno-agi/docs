...
Workflow History enables your Agno workflows to remember and reference previous conversations, transforming isolated executions into continuous, context-aware interactions. Instead of starting fresh each time, your workflows can:
...
<Tip>
Note that this feature is different from `add_history_to_context` in the sense that it does not add the history of a particular agent or team of the workflow's step 
but rather the full workflow history to either all or some step
</Tip>

## How It Works

When workflow history is enabled, previous conversation turns are automatically injected into agent/team inputs as structured context:
...
<Note>
`step_input.get_workflow_history(num_runs=3)` and `step_input.get_workflow_history_context(num_runs=3)` are the helper functions which are available 
to access the history regardless if `add_workflow_history_to_steps` is enabled or not.
</Note>
...
```python
# ✅ Good: Limit history to relevant recent runs
workflow = Workflow(
    add_workflow_history_to_steps=True,
    num_history_runs=3  # Only include last 3 conversations
)

# ❌ Avoid: Unlimited history can bloat context
workflow = Workflow(
    add_workflow_history_to_steps=True,
    # num_history_runs not set = includes ALL history
)
```
