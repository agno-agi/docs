...
If you are running an agent or team within the custom function step, you can enable streaming on the [AgentOS chat page](/agent-os/introduction#chat-page) by setting `stream=True` and `stream_intermediate_steps=True` when calling `run()` or `arun()` and yielding the events.
