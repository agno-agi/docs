...
If you are running an agent or team within the custom function step, you can enable streaming on chat page by setting `stream=True` and `stream_intermediate_steps=True` by setting `stream_intermediate_steps=True` when calling `run()` or `arun()` and yielding the events.

<Note>
Since by default AgentOS uses asynchronous streaming behaviour so we should keep the custom function step async and use `.arun()` instead of `.run()` for running agent or team within for proper streaming support.
</Note>
