...
Just like with other HITL patterns, you can continue the run later using the `run_id` and `updated_tools`:

```python
run_response = agent.run("List my files")
...
if run_response.is_paused:
    ...
    # Execute the tools later
    for tool in tools:
        if tool.external_execution_required:
            tool.result = execute_tool_externally(tool)
    
    # Continue with the stored run_id
    response = agent.continue_run(run_id=run_id, updated_tools=tools)
```

This is useful when you need to queue external execution tasks or when execution might take a long time.
...
<CardGroup cols={2}>
  <Card
    title="External Execution Examples"
    icon="plug"
    href="/examples/concepts/agent/human_in_the_loop/external_tool_execution"
  >
    View complete examples of external execution patterns
  </Card>
  <Card
    title="HITL Overview"
    icon="book"
...
- View [Examples](/examples/concepts/agent/human_in_the_loop/external_tool_execution)
