...
The same `UserInputField` structure is used in [Dynamic User Input](/concepts/hitl/dynamic-user-input), where the agent dynamically creates these fields when it needs information.
</Note>
...
<Note>
[Dynamic User Input](/concepts/hitl/dynamic-user-input#async-support) also supports async patterns with the same methods.
</Note>
...
Like with user confirmation, you can continue a paused run from a different context using the `run_id`:
...
if run_response.is_paused:
    ...
    for tool in run_response.tools_requiring_user_input:
        for field in tool.user_input_schema:
            if field.value is None:
                field.value = "user@example.com"  # Or get from user
...
<CardGroup cols={2}>
  <Card
    ...
    icon="keyboard"
    href="/examples/concepts/agent/human_in_the_loop/user_input_required"
  >
    View complete examples of user input patterns
  </Card>
  <Card
    title="HITL Overview"
    icon="book"
  ...
  >
    Let agents request input dynamically when needed
  </Card>
  <Card
...
- View [Examples](/examples/concepts/agent/human_in_the_loop/user_input_required)
