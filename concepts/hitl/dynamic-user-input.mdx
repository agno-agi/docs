...
Dynamic user input lets your agent decide when it needs information from the user and proactively request it during execution. Unlike the [User Input](/concepts/hitl/user-input) pattern where you predefine which tools need user input, this pattern gives the agent autonomy to pause and ask for information whenever it realizes it doesn't have what it needs.
...
These fields then appear in `tool.user_input_schema` as `UserInputField` objects that you can iterate through and fill. For a detailed breakdown of the `UserInputField` structure, see [Understanding UserInputField](/concepts/hitl/user-input#understanding-userinputfield).
...
The agent can pre-fill some fields based on the conversation context. This works the same way as in [User Input](/concepts/hitl/user-input#handling-pre-filled-values)—always check `field.value` before prompting:
...
For a more detailed explanation of how pre-filled values work, see the [Handling Pre-Filled Values](/concepts/hitl/user-input#handling-pre-filled-values) section in the User Input documentation.
...
For more details on async, streaming, and run_id continuation patterns, see the [User Input](/concepts/hitl/user-input) documentation which covers these in depth.
...
**Use [User Input](/concepts/hitl/user-input) when:**
...
<CardGroup cols={2}>
  <Card
    ...
    icon="comments"
    href="/examples/concepts/agent/human_in_the_loop/agentic_user_input"
  >
    View complete examples of dynamic input patterns
  </Card>
  <Card
    title="HITL Overview"
    icon="book"
  ...
  >
    Learn about other human-in-the-loop patterns
  </Card>
  <Card
    title="User Input"
    icon="keyboard"
  ...
  >
    Gather specific predefined information from users
  </Card>
  <Card
  ...
  >
    Require explicit user approval before executing tools
  </Card>
</CardGroup>

## Developer Resources

- View [Examples](/examples/concepts/agent/human_in_the_loop/agentic_user_input)
