...
The `max_tool_calls_in_context` parameter can used to keep only the N most recent tool calls in context. 
...
agent = Agent(
    ...
    add_history_to_context=True,
    max_tool_calls_in_context=3,  # Keep only last 3 tool calls in context
)
...
<Note>
**Important:** `max_tool_calls_in_context` filters tool calls from the runs loaded by `num_history_runs`. Your database always contains the complete history.
</Note>

See the [full example](/examples/concepts/agent/context_management/forget_tool_calls) for a complete demonstration.
