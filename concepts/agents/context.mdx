...
agent = Agent(
    ...
    add_history_to_context=True,
    max_tool_calls_in_context=3,  # Keep only last 3 tool calls
)
...
agent.print_response("What's the weather in Berlin?")  # Tokyo is now filtered out
```

<Tip>
  A value of 3-10 is typically sufficient for most use cases.
</Tip>

See the [full example](/examples/concepts/agent/context_management/forget_tool_calls) for more details.
