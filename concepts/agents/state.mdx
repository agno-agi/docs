---
title: Agent State
description: Learn how to share data between runs in a session.
---

Your Agents will often need to access certain data during a session. It could be a todo list, the user profile, or anything else.

When this data needs to remain accessible across runs, or needs to be updated during the session, you want to consider it **state data**.

State data will be accessible from tool calls, pre-hooks and post-hooks, and other functions that are part of the Agent run. You will also be able to use it in the system message, to ultimately present it to the Model.

## What is State data?

A simple yet common use case for Agents is to manage lists for a user. For example: a shopping list, a todo list, or a wishlist.

Because this list should be available across the session, and will be updated at times, it should be considered **state data**.

The Agent will be able to access and update that data in tool calls, hooks, and also to expose it to the Model via the system message.
The session state will be persisted in the database if one is available to the Agent.
...
- The `session.get_session_state()` method retrieves the session state of a particular session from the database
...
Now that we understand what State data is, let's see how it works:
...
- You can access the state data in tool calls and other functions, via `run_context.session_state`.
- You can use any data in your `session_state` in the system message, by referencing it in the `description` and `instructions` parameters.
- You can pass `session_state` to `agent.run()`. This will take precedence over the Agent's default state for that run.
- The `session_state` will be stored in your database. Subsequent runs **within the same session** will load the state from the database.
- When loading the `session_state` from the database, the current `session_id` will be used to load the right data for that session.
...
<Note>
  The `run_context` object is automatically passed to the tool as an
  argument. Any updates to `run_context.session_state` will beautomatically persisted in the database andreflected in the shared state.
</Note>

<Check>
  Session state is also shared between members of a team when using `Team`. See [Teams](/concepts/teams/state) for more information.
</Check>
