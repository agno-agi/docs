...
The `max_tool_calls_in_context` parameter can be used to keep only the N most recent tool calls in context. 
...
team = Team(
    ...
    add_history_to_context=True,
    max_tool_calls_in_context=5,  # Keep only last 5 tool calls in context
...
<Note>
**Important:** `max_tool_calls_in_context` filters tool calls from the runs loaded. Your database always contains the complete history.
</Note>
