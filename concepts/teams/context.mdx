...
The `max_tool_calls_from_history` parameter can be used to keep only the N most recent tool calls in context. 
...
<Note>
**Important:** `max_tool_calls_from_history` filters tool calls from the runs loaded. Your database always contains the complete history.
</Note>
