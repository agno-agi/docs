...
To manage context size and token usage we can set `add_history_to_context`. This keeps only the last N tool calls in context.
...
web_agent = Agent(
    ...
    instructions="You are a web researcher. You are tasked with searching the web for information on a given topic. You are to find as much information as possible.",
    tools=[DuckDuckGoTools()],
)
...
team = Team(
    ...
    tools=[DuckDuckGoTools()],  # Team also has search tools
    add_history_to_context=True,
    num_history_runs=8,  # limit the number of history responses to add to the context
    max_tool_calls_in_context=5,  # Keep last 3 tool calls in context
...
team.print_response("Search for startups")  # First search is now filtered out
```

<Tip>
  For teams, consider a slightly higher value (5-15) to account for multiple members making tool calls.
</Tip>

See the [full team example](/examples/concepts/teams/context_management/forget_tool_calls) for a detailed demonstration.
