...
| `parallel_extract` | Extract content from specific URLs using Parallel's Extract API. Parameters include `urls` (list) for URLs to extract from, `objective` (str) to guide extraction, `search_queries` (list) for targeting relevant content, `excerpts` (bool or dict, default=True) to include text snippets, and `full_content` (bool or dict, default=False) to include complete page text. Returns JSON formatted string with extracted content in clean markdown format. |

## Developer Resources

- View [Example](/examples/concepts/tools/search/parallel)
