---
title: Notion Tools
description: Tools for interacting with Notion Database.
---
...
| ------------------- | ------ | ------- | -------------------------------------------------------- |
| `api_key` | `str` | `None`  | Notion API key (integration token). Uses `NOTION_API_KEY` env var if not provided.              |
| `database_id`     | `str` | `None`  | The ID of the database to work with. Uses `NOTION_DATABASE_ID` env var if not provided. |
