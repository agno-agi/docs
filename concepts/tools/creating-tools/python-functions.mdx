...
sidebarTitle: Python Functions
description: Learn how to create tools with python functions.
keywords: python functions, @tool decorator, custom tools, function calling, tool hooks, caching
---
...
Use Python functions as tools when:
- You need a simple, standalone tool
- The tool doesn't require shared state or configuration
- You want the quickest way to add custom functionality

For multiple related tools that share state (like database connections), consider creating a [Toolkit](/concepts/tools/creating-tools/toolkits) instead.

## Basic Example

Here's how to use a `get_top_hackernews_stories` function as a tool:
...
<CardGroup cols={3}>
  ...
  >
    Add validation and logging to your tools
  </Card>
  <Card
  ...
  >
    Group related tools into reusable toolkits
  </Card>
  <Card
