...
keywords: tool hooks, pre hooks, post hooks, validation, logging, function interceptors
---
...
Use tool hooks when you need to:
- **Add logging or monitoring** - Track tool usage, performance, or errors
- **Validate inputs or outputs** - Enforce business rules before or after execution
- **Add authentication checks** - Verify permissions before allowing tool access
- **Transform data** - Modify arguments or results dynamically
- **Implement rate limiting** - Control how often tools can be called

Tool hooks are optional - only add them when you need this additional control layer.
...
Pre and post hooks are a simpler alternative to tool hooks when you only need to run logic before or after a tool call.

**Key differences:**
- **Tool hooks** wrap the entire tool execution and give you full control
- **Pre/post hooks** run at specific points but don't wrap execution
- Pre/post hooks receive a `FunctionCall` object instead of raw parameters
...
<CardGroup cols={3}>
  ...
  >
    Learn about the @tool decorator and tool_hooks parameter
  </Card>
  <Card
  ...
  >
    Add hooks to custom toolkit tools
  </Card>
  <Card
