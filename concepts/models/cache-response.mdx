...
Response caching allows you to cache model responses locally, avoiding redundant API calls when the same query is made multiple times. This feature is particularly useful during development, testing, and scenarios where you expect repeated queries.

<Note>
**Response Caching vs. Prompt Caching**: Response caching (covered here) caches the entire model response locally to avoid API calls. [Prompt caching](/examples/models/anthropic/prompt_caching) (Anthropic-specific) caches the system prompt on the provider's side to reduce processing time and costs.
</Note>
...
- **Development and Debugging**: Test your agent logic without waiting for API responses
- **Unit Testing**: Ensure consistent, reproducible test results
- **Demo and Prototyping**: Run demos without incurring API costs for repeated queries
- **Iterative Refinement**: Test different parts of your system without re-querying the model
...
Response caching works seamlessly with:

- **Synchronous responses**: `agent.run()`
- **Asynchronous responses**: `await agent.arun()`
- **Streaming responses**: `agent.run(stream=True)`
- **Async streaming**: `await agent.arun(stream=True)`

For streaming responses, Agno caches the complete accumulated response and replays it when the cache is hit.
