...
Without memory, every conversation starts from scratch. Your agent might ask the same questions repeatedly or miss important context from previous interactions. With memory enabled, agents can learn about users over time and provide increasingly personalized, context-aware responses.
...
With `enable_user_memories=True`, your agent automatically creates and updates memories after each conversation. It extracts relevant information, stores it, and recalls it when needed—no manual intervention required.
...
<Note>
**Important:** Don't enable both `enable_user_memories` and `enable_agentic_memory` at the same time—they're mutually exclusive. While nothing will break if you set both, `enable_agentic_memory` will always take precedence and `enable_user_memories` will be ignored.
</Note>

## Storage: Where Memories Live

Memories are stored in whatever database you connect to your agent. Agno supports all major database systems—Postgres, SQLite, MongoDB, and more. Check the [Storage documentation](/concepts/db/overview) for the full list of supported databases and setup instructions.

By default, memories are stored in the `agno_memories` table (or collection, for document databases). If this table doesn't exist when your agent first tries to store a memory, Agno creates it automatically—no manual schema setup required.
...
<CardGroup cols={2}>
  <Card title="Database Setup" icon="database" href="/concepts/db/overview">
    Learn about database configuration and supported options
  </Card>
  <Card title="Advanced Memory" icon="brain" href="/concepts/memory/advanced">
    Deep dive into custom memory strategies and optimization
  </Card>
</CardGroup>
