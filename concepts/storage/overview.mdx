...
- [Knowledge](/concepts/knowledge/content_db)
- [Evals](/concepts/evals/overview)
- <Tooltip tip="Culture is an experimental feature." cta="Read the Culture guide" href="/concepts/culture/overview">Culture</Tooltip>


```mermaid
flowchart LR
  User["User"] -->|"messages"| Agent["Agent"]

  subgraph AgenticApp["Agentic application"]
    Agent -->|"store / load sessions, memories, etc."| Storage[("Storage (DB)")]
  end

  Storage --> Sessions["Sessions"]
  Storage -->Memories["User Memories"]
  Storage --> OtherData["Knowledge, Evals, Culture"]
```


## Adding storage to your Agent

To enable session persistence on your Agent, just set up a database and provide it to the Agent:
```python
...
When building production-ready agentic applications, storage will often be a very important feature. This is because it enables you to:
...
## Supported databases
 ...
 Databases we currently support for storage:
...
<CardGroup cols={3}>
  <Card
    ...
    icon="database"
    iconType="duotone"
    href="/concepts/storage/dynamodb"
  >
  ...
  <Card
    ...
    icon="database"
    iconType="duotone"
    href="/concepts/storage/firestore"
  >
  ...
  <Card
    ...
    icon="database"
    iconType="duotone"
    href="/concepts/storage/postgres"
  >
  ...
  <Card
    ...
    icon="database"
    iconType="duotone"
    href="/concepts/storage/singlestore"
  >
  ...
  <Card
    ...
    icon="database"
    iconType="duotone"
    href="/concepts/storage/supabase"
  >
...
<Note>
We also support using an [In-Memory](/concepts/storage/in-memory) database. This is not recommended for production, but perfect for demos and testing.
</Note>

You can see a detailed list of [examples](/examples/concepts/storage) for all supported databases.
