...
<Warning>
In v2.7.2, the Curator reads a `memories` field from the User Profile store, but the default `UserProfile` schema doesn't define one. `prune()` and `deduplicate()` return `0` unless you add a `memories` field to a custom profile schema and populate it yourself. See [User Memory](/learning/stores/user-memory#curation) for the current workaround.
</Warning>
