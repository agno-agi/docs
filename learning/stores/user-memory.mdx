...
<Warning>
The Curator does not work with the User Memory Store in v2.7.4. `lm.curator.prune()` and `lm.curator.deduplicate()` read the User Profile store, which has no `memories` field, so both calls return 0 without removing any user memories. To remove memories, use the agentic `update_user_memory` tool, which supports delete operations.
</Warning>
