...
Wire these in through the matching config, the same way as `UserProfileConfig`: `EntityMemoryConfig(schema=CompanyEntity)` or `LearnedKnowledgeConfig(schema=TechnicalInsight)`.
