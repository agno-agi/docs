...
Use `output_model` to delegate final response formatting to a different model.
The primary model handles reasoning and tool calls, then the output model formats the result.

If the primary model does not support structured output, use `output_model` and `output_schema`. 
For custom output styling, use `output_model_prompt`.
...
| Need custom formatting style | `output_model` + `output_model_prompt` |
