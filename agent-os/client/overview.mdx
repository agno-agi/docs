...
|--------|----------|----------|
| `AgentOSClient` | Connect to Agno AgentOS instances with full feature access (sessions, knowledge, memories, traces) | REST |
| `A2AClient` | Connect to any A2A-compatible server for cross-framework agent communication | A2A (REST or JSON-RPC) |
...
Both clients are included in the `agno` package:
