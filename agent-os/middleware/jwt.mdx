...
When `user_isolation=True`, non-admin callers are scoped to their own `user_id` (from the JWT `sub` claim) throughout AgentOS for user-owned data. Deployment-wide resources remain protected by RBAC scopes. Callers holding `admin_scope` bypass isolation. See [Per-User Data Isolation](/agent-os/security/authorization/user-isolation) for the full behavior.
...
| `user_isolation` | Opt in to per-user data isolation throughout AgentOS. When `True`, AgentOS uses the JWT `sub` claim as the `user_id` for every non-admin caller: user-owned reads are scoped to it and user-owned writes are coerced to it. Deployment-wide resources remain protected by RBAC scopes. Callers holding `admin_scope` bypass isolation. | `False` |
