...
Use cases:
- Authenticate agent runs with `user_id` and `session_id` from tokens
- Filter `/sessions` endpoints by `user_id` automatically
- Inject `dependencies` from token claims into agent runs for tool access
