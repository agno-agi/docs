...
When `user_isolation=True`, user isolation applies throughout AgentOS to user-owned data and resources. Non-admin callers are scoped to their own `user_id` from the JWT `sub` claim. Callers holding `admin_scope` bypass isolation. See [Per-User Data Isolation](/agent-os/security/authorization/user-isolation) for the full behavior.
