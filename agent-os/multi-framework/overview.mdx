...
|------------|--------|-------|
| `AgentOS(agents=[...])` registration | Supported | Adapters satisfy `AgentProtocol` |
| `/agents` and `/agents/{id}/runs` endpoints | Supported | Same routes as native agents |
| SSE streaming | Supported | Token and tool call events emitted by adapters |
| Session persistence | Supported | When `db` is set on `AgentOS` |
| Standalone `.run()` / `.print_response()` | Supported | Sync and async |
| Tool call visibility in the UI | Supported | Wrapped as Agno tool events |
| Use as a `Team` member | Not supported | Agno `Team` orchestration is built around the native `Agent` |
| Memory, knowledge, dependencies, hooks, guardrails | Not supported | These are Agno-SDK concepts wired into the native `Agent` |
| Structured input and output | Not supported | Use the framework's own typing (DSPy signatures, LangGraph state) |
| Skills, reasoning, learning | Not supported | Native `Agent` only |
