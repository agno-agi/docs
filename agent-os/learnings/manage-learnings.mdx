...
sidebarTitle: "Manage Learnings"
description: "Read and manage learning records through the AgentOS /learnings endpoints."
---
...
The learning stores key records by a deterministic ID derived from their identity fields, not a random UUID. `POST` computes the **same** ID for the identity-keyed types, so a record created through the API reconciles with what the agent reads and writes without creating orphaned or duplicate rows.
