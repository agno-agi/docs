...
description: "Serve AgentOS as an MCP server at /mcp, with eight built-in tools, custom tools, and per-call authorization via MCPServerConfig."
...
The MCP server needs `fastmcp`. Building the app without it raises an `ImportError`, so install the extra:
