...
<Note>
With `run_hooks_in_background=True` on AgentOS, **all** hooks for all agents run in the background. Use the [`@hook` decorator](/agent-os/usage/background-hooks-decorator) for more granular control over which hooks run in the background.
</Note>
