...
AgentOS validates the JWT on protected requests, then checks its scopes against the permissions each endpoint requires. Health, information, and API documentation routes are public by default. This controls who can access and run your agents, teams, and workflows.
...
Set a verification key, then enable authorization when initializing AgentOS:
...
<CardGroup cols={2}>
  ...
  <Card
    ...
    icon="users"
    href="https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/05_agent_os/rbac/symmetric/user_isolation.py"
  >
