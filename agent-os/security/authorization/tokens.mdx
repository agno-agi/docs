...
AgentOS reads the JWT from the `Authorization: Bearer <token>` header on protected requests. The default public routes are `/`, `/health`, `/info`, `/docs`, `/redoc`, `/openapi.json`, and `/docs/oauth2-redirect`. Tokens can come from the AgentOS control plane or your own backend.
