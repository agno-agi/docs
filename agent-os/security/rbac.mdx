...
<Badge icon="code-branch" color="orange">
    <Tooltip tip="Introduced in v2.3.9" cta="View release notes" href="https://github.com/agno-agi/agno/releases/tag/v2.3.9">v2.3.9</Tooltip>
</Badge>
...
flowchart LR
    subgraph Clients[Client Applications]
        CA[Your own application]
    end

    subgraph ControlPlane[Control Plane]
        CP[AgentOS Control Plane]
    end

    subgraph API[AgentOS API]
        V[JWT Verifier]
        R[Agents / Teams / Workflows]
    end

    CP -- "API Request + JWT" --> V
    CA -- "API Request + JWT" --> V
    V -- "Validates Scopes" --> R
...
agent_os = AgentOS(
    ...
    agents=[agent],
    authorization=True,
    authorization_config=uthorizationConfig(
      verification_key="your-jwt-verification-key",
      algorithm="RS256",
  ),
)
```
