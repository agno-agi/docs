...
<AccordionGroup>
  ...
  <Accordion title="403 on webhook">
    ...
    **Fix:** Verify `WHATSAPP_APP_SECRET` matches the value under **App Settings > Basic** in your [Meta App Dashboard](https://developers.facebook.com/apps/). Use `WHATSAPP_SKIP_SIGNATURE_VALIDATION=true` only for isolated localhost payload tests that are not network-reachable.
  </Accordion>
...
<CardGroup cols={2}>
  ...
  <Card title="Setup Guide" icon="rocket" href="/agent-os/interfaces/whatsapp/setup">
