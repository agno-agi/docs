...
When using the AgentOS API, you can apply advanced filter expressions to precisely control which knowledge base documents your agents search. FilterExpressions serialize to JSON and are automatically reconstructed server-side for powerful, programmatic filtering.

## Overview

FilterExpressions provide a composable way to filter knowledge using complex logical conditions through HTTP APIs. They offer the same power as Python-based filtering but work seamlessly over REST endpoints.
...
FilterExpr objects use a dictionary format with an `"op"` key that distinguishes them from regular dict filters:
...
<Note>
The presence of the `"op"` key tells the API to deserialize the filter as a FilterExpr. Regular dict filters (without `"op"`) continue to work for backward compatibility.
</Note>
...
- Use **FilterExpressions** when you need OR logic, exclusions (NOT), or range queries (GT/LT)
</Tip>
...
### FilterExpressions (Advanced Approach)
...
### Multiple FilterExprs

Send multiple filters as a JSON array:
...
| OR conditions | Not possible | `OR(EQ("priority", "high"), EQ("urgent", True))` | **FilterExpr** - Only option |
| Range queries | Not possible | `AND(GT("price", 10), LT("price", 100))` | **FilterExpr** - Only option |
| Exclusions (NOT) | Not possible | `NOT(EQ("status", "archived"))` | **FilterExpr** - Only option |
| Complex nested logic | Not possible | `OR(AND(...), NOT(...))` | **FilterExpr** - Only option |
...
### Basic FilterExpr Operators

When you need more than simple AND logic, use FilterExpr operators:
...
The Agno API automatically handles FilterExpr deserialization:

<Steps>
  <Step title="Client Serialization">
    FilterExpr objects are converted to dict format with `to_dict()`
  </Step>
  ...
  <Step title="Auto-Detection">
    API detects FilterExpr format by checking for `"op"` key
  </Step>
  <Step title="Reconstruction">
    FilterExpr objects are reconstructed from the dict structure
  </Step>
  <Step title="Agent Processing">
    Agent uses FilterExpr objects for knowledge base filtering
  </Step>
</Steps>

### Detection Logic

The API differentiates between FilterExpr and dict filters:

```python
# FilterExpr format (has "op" key)
{"op": "EQ", "key": "status", "value": "published"}
...
## Choosing Between Dict and FilterExpr
...
def create_filter(filter_type="simple"):
    ...
    if filter_type == "simple":
        ...
        # FilterExpr: Complex logic with comparisons
        return AND(
            EQ("category", "tech"),
            GT("views", 1000),
            NOT(EQ("status", "archived"))
