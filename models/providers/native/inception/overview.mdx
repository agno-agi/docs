...
**Note**: Inception's OpenAI-compatible endpoint does not implement native `json_schema` structured outputs, so `supports_native_structured_outputs` is set to `False`. Use `use_json_mode=True` for structured output.
