...
description: Use OpenAI GPT and reasoning models with Agno agents.
---

GPT models are used as the default LLM by **Agents**. OpenAI supports a variety of models. See the [full list](https://platform.openai.com/docs/models).
...
Prompt caching happens automatically with the `OpenAIChat` model. Read more about how OpenAI handles caching in [their docs](https://platform.openai.com/docs/guides/prompt-caching).
...
| `reasoning_effort`           | `Optional[ReasoningEffort]`    | `None`                 | The reasoning effort level for reasoning models. Named values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; any other string is passed through, and model support varies |
| `verbosity`                  | `Optional[Verbosity]`          | `None`                 | Controls verbosity level of reasoning models. Named values are `low`, `medium`, and `high`; any other string is passed through |
...
| `service_tier`               | `Optional[ServiceTier]`        | `None`                 | Service tier to use. Named values are `auto`, `default`, `flex`, `scale`, `priority`, `fast`, and `ultrafast` (`ultrafast` is Responses-only); any other string is passed through. Available tiers vary by account |
