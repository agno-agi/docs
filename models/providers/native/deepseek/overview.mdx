...
DeepSeek provides endpoints for large language models.
...
For agent scenarios, DeepSeek recommends `reasoning_effort="max"`. The API default is `"high"`. While thinking is active, `temperature`, `top_p`, `presence_penalty` and `frequency_penalty` are ignored by the API.
