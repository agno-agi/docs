...
3. Enable Vertex AI API and set the project ID environment variable (alternatively, you can set `project_id` on the `Gemini` model):
...
This example uses Pillow to save the image. Install it with `uv pip install pillow`.
...
`grounding=True` uses the legacy grounding API. Prefer `search` on Gemini 2.0+ models:
...
agent.print_response("Summarize the top stories on HackerNews.")
```
...
|-----------|------|---------|-------------|
| `id` | `str` | `"gemini-3.7-flash"`     | The id of the Gemini model to use |
...
| `credentials` | `Optional[Credentials]` | `None` | Google Cloud service account credentials for Vertex AI |
...
| `parallel_api_key` | `Optional[str]` | `None` | Parallel API key (defaults to `PARALLEL_API_KEY` env var) |
| `parallel_config` | `Optional[Dict[str, Any]]` | `None` | Custom configuration for Parallel search, e.g. domain filtering |
...
| `client` | `Optional[Client]` | `None` | Preconfigured `google.genai` client to use instead of creating one |
