...
description: Upload a local image file to Gemini and pass it to an agent.
---
...
<Steps>
  ...
  <Step title="Download the image">
    Save the image next to the script, at `cookbook/90_models/google/gemini/`:
    ```bash
    wget -P cookbook/90_models/google/gemini https://upload.wikimedia.org/wikipedia/commons/b/bf/Krakow_-_Kosciol_Mariacki.jpg
    ```
  </Step>
  ...
  <Step title="Install dependencies">
    ```bash
    uv pip install -U google-genai google-generativeai ddgs agno
    ```
  </Step>
