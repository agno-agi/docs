...
<Note>
  Agno v3.0 sends structured output through Anthropic's `output_config.format`
  request shape, which requires `anthropic>=0.77.0` (the `agno[anthropic]`
  extra pins it). Earlier Agno versions used the legacy `output_format`
  request, which `anthropic` 1.0.0 removed.
</Note>
