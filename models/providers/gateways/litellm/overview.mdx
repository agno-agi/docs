...
2. **[Proxy Server integration](/models/providers/gateways/litellm-openai/overview)** - Using LiteLLM as an OpenAI-compatible proxy
...
Set up your API key:
Agno checks `LITELLM_API_KEY` first. If it's not set, it falls back to the provider-specific key for the model you're using, e.g. `OPENAI_API_KEY` for an OpenAI model.
