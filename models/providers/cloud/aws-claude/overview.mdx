...
| `api_key`                | `Optional[str]`            | `None`                                       | Not currently supported. Setting it (or the AWS_BEDROCK_API_KEY env var) raises an error; use IAM credentials or a boto3 session instead |
| `aws_access_key`         | `Optional[str]`            | `None`                                       | The AWS access key to use (defaults to AWS_ACCESS_KEY_ID, then AWS_ACCESS_KEY env var)                  |
| `aws_secret_key`         | `Optional[str]`            | `None`                                       | The AWS secret key to use (defaults to AWS_SECRET_ACCESS_KEY, then AWS_SECRET_KEY env var)              |
| `aws_session_token`      | `Optional[str]`            | `None`                                       | The AWS session token for temporary credentials (defaults to AWS_SESSION_TOKEN env var)                 |
