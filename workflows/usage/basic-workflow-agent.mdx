...
description: Let a WorkflowAgent run workflow steps or answer from prior workflow history.
---

Use `WorkflowAgent` to create a conversational workflow. The agent decides whether to run the workflow steps or answer directly from previous run history, so a follow-up like "What was Rocky's personality?" gets answered without triggering a fresh run.
