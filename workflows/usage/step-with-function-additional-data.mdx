...
description: Pass metadata and contextual information to steps via `additional_data`.
---

Pass metadata and contextual information to steps via `additional_data`. Workflow logic stays separate from configuration, and steps can adapt to context like user details and priority.

Read the supplied dictionary from `step_input.additional_data` inside a custom executor.
