...
| `reasoning_effort`           | `Optional[str]`                | `None`                 | Model-dependent reasoning effort. API values are `none`, `minimal`, `low`, `medium`, `high`, `xhigh`, and `max`; model support varies. See [Create chat completion](https://developers.openai.com/api/reference/resources/chat/subresources/completions/methods/create). |
