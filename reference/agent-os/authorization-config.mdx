...
description: "Parameter reference for AuthorizationConfig, which configures JWT verification for AgentOS RBAC authorization."
---
...
| `user_isolation` | `bool` | `False` | Opt-in per-user data isolation throughout AgentOS. When `True`, non-admin callers can only read and write user-owned data associated with their JWT `sub` claim. Deployment-wide resources remain protected by RBAC scopes. |
