...
|-----------|------|---------|-------------|
| `verification_key` | `Optional[str]` | `JWT_VERIFICATION_KEY` env var | Key for JWT verification. For RS256, use public key. For HS256, use shared secret. |
| `secret_key` | `Optional[str]` | `None` | **(Deprecated)** Use `verification_key` instead. |
