...
| `capabilities` | `Dict[str, Any]` at runtime | A2A capabilities object. Agno servers return flags such as `streaming` and `pushNotifications`. The v2.7.4 dataclass annotates this field as `List[str]`, but both client parsers return the server object unchanged |
