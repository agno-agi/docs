...
Every command accepts `--json`. After command-line parsing succeeds:

- Successful results and command or runtime errors emit exactly one JSON document on stdout and nothing else.
- Interactive prompts are disabled. Runs that would need one fail with an error.
- Command and runtime errors emit `{"error": "<message>", "hint": "<optional hint>"}` and exit non-zero.

Typer handles usage and parsing errors before the command runs. Unknown commands, unknown flags, missing arguments, and invalid option values exit with code `2` and write diagnostics to stderr. These errors do not emit JSON, even when `--json` is present.
...
Scaffolds a project by shallow-cloning a template repository (`git clone --depth 1`, 300s timeout) into `./<name>`, removing its `.git` directory, and copying `example.env` to `.env` with mode `0600` when that file exists. It requires `git` and fails if the directory already exists. `<name>` must be a single path segment of letters, digits, `-`, and `_`.

Bare `agno create` prompts for a template and project name in an interactive terminal. The prompt defaults are `agentos-docker` and `agentos`. JSON and non-interactive runs require an explicit name.
...
Revoking is irreversible. The worker that processes the revoke rejects the token immediately. Other workers reject it after their verification cache expires, which takes up to 30 seconds by default. Interactive runs confirm first. `--yes`, `--json`, and non-TTY runs skip the prompt.
