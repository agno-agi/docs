...
description: "Give agents text files for durable decisions, records, and checkpoints."
keywords: [filesystem, agent filesystem, durable state, working state, checkpoints, agent notes]
---

`FileSystem` gives an agent a text store for working notes it writes and maintains. Use a persistent backend to retain those notes across runs.
...
| State | Stores | Scope | Use it for |
|-------|--------|-------|------------|
| [Memory](/memory/overview) | Curated facts about a user | `user_id` across sessions | Preferences, responsibilities, recurring goals |
| [Session state](/state/overview) | Application data managed by code or tools | `session_id` | Carts, task progress, counters |
| [Knowledge](/knowledge/overview) | Reference material retrieved at runtime | Application-defined | Documents, policies, domain information |
| FileSystem | Text the agent writes for future runs | Namespace within one backend | Decisions, maintained notes, checkpoints, processed records |
...
Import `PostgresDb` from `agno.db.postgres` and `LocalFileSystem` from `agno.fs.local`.

SQLite and Postgres store one row per namespace and path. Postgres uses the `fs` schema and `agno_fs` table by default.

Database-backed FileSystem accepts synchronous SQLite and Postgres database handles. SQLite 3.35.0 or newer is required. Async FileSystem methods run the synchronous backend operations in worker threads.

## Isolate or Share Files

The default namespace is `"default"`. FileSystem instances that use the same backend and omit `namespace` share that store.
...
| Configuration | Tools |
|---------------|-------|
| `fs.tools()` | `read_file`, `write_file`, `append_file`, `replace_lines`, `list_files`, `search_content`, `move_file` |
| `fs.tools(read_only=True)` | `read_file`, `list_files`, `search_content` |
| `fs.tools(allow_delete=True)` | Default tools plus `delete_file` |
| `fs.tools(include_tools=[...])` | Explicit selection from the full surface, including `check_lines` |

Add `check_lines` to the default tool surface for exact processed-record sets:

```python
record_tools = fs.tools(
    include_tools=[
        "read_file",
        "write_file",
        "append_file",
...
Attach one file-like toolkit to an agent. Wrap one toolkit in a sub-agent when an application needs both FileSystem and a local workspace.
