...
- Knowledge has been completely reworked. See full details in the [Knowledge](/context/knowledge/overview) section.
...
- Import locations for `embedder`, `document`, `chunking`, `reranker` and `reader` have been moved to `agno.knowledge`. See [examples](/context/knowledge/overview) for more details.
...
- Added support for custom loggers. See the [documentation](/features/custom-logging) for more details.
