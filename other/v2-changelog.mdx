...
description: "Full list of class, parameter, and behavior changes introduced in Agno v2.0."
---
...
- We have made significant changes to the inner workings of `Agent`, `Team` and `Workflow` to make them completely stateless.
- This means that `agent_session`, `session_metrics`, `session_state`, etc. should not be seen as stateful variables that would be updated during the course of a run, but rather as "defaults" for the agent if they can be set on initialization.
...
- General:
  - Since Agents and Teams are now stateless, using attributes from the agent/team object inside a function will give you access to the attributes set on initialization of that agent/team. E.g. `agent.session_state` should not be used, instead `session_state` can now be directly accessed and would have the "current" state of the session.
...
<Accordion title="Updates to Workflow Class">
  ...
  - This means the import location for "Workflows v2" is now `agno.workflow`.
  - `extra_data` -> `metadata`
