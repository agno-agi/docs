...
# db.cleanup_legacy_runs_field(force=True)  # Mongo / Redis / Valkey / Firestore / Dynamo / JSON adapters
```
...
- **`update_schedule` is restricted to a column allow-list**: it now writes only
  ...
  `enabled`, `next_run_at` and `disabled_reason`. Passing anything else — such
  as `user_id` or a provenance column — raises a `ValueError` instead of
  silently repointing the row's owner or target.
- **Removed toolkit methods**: `DuckDuckGoTools.duckduckgo_search` ->
  ...
  `BrightData.get_screenshot` no longer takes `output_path`.
...
- **Other toolkit renames**: `SeltzTools.max_documents` -> `max_results` (and
  `seltz>=1.2.0` is now required); `BrandfetchTools` drops `async_tools`;
...
- **AgentOS MCP config**: `AgentOS(enable_mcp_server=..., mcp_config=...)` ->
  `mcp_server=` (a bool or `MCPServerConfig`). Passing `websocket=` no longer
  implies `enable_websocket=True`.
...
- FileTools: .check_escape(             -> ._check_path(
  (do NOT rename LocalFileSystemTools.check_escape - that one still exists)
- BrightData.get_screenshot(...): drop any output_path= argument
- PgVector: remove any enable_prefix_matching= argument (the helper is gone)
...
JUDGMENT section listing every finding from steps 1b, 3, 5, 6 and 7 that needs my
