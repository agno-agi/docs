...
```bash
pip install -U "agno[os,agui]" openai
```

Otherwise, you can install the latest version of Agno v2 by running:
```bash
pip install "agno[os,agui]" openai
```

The `os` and `agui` extras and `openai` cover the examples in this guide.
...
Use our migration script: [`libs/agno/migrations/v1_to_v2/migrate_to_v2.py`](https://github.com/agno-agi/agno/blob/v2.7.4/libs/agno/migrations/v1_to_v2/migrate_to_v2.py)
...
This is how it looks now, in v2:
...
This is how Storage looks in v1:
...
This is how Memory looks in v1:
...
Our `Playground` has been deprecated. Our new [AgentOS](/agent-os/introduction) offering will substitute all use cases.
