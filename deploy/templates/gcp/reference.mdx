...
| Tail logs | `gcloud run services logs read agent-os --project "${GCP_PROJECT_ID:-$(gcloud config get-value project)}" --region "${GCP_REGION:-us-central1}" --limit 100` |
...
| `RUNTIME_ENV` | No | `prd` | `dev` sets `authorization=False`, which disables AgentOS scope enforcement. Configured JWT environment variables continue to enable JWT validation. Compose sets `dev` locally; keep `prd` on Cloud Run. |
