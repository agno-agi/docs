...
For a deployed platform, point the same command at your domain: `uvx agno connect --url https://<your-domain>`. When `MCP_CONNECT_SECRET` is set, `/mcp` is OAuth-protected and each client completes a one-time sign-in. Otherwise the command mints a service-account token (`agno_pat_...`) for the connection.
