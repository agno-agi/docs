...
Choose AWS when your team wants AgentOS, PostgreSQL, secrets, logs, and networking in its own AWS account. The [agentos-aws](https://github.com/agno-agi/agentos-aws) template uses Docker locally and deploys AgentOS with ECS Express Mode and RDS for production.
...
- [AWS CLI v2](https://docs.aws.amazon.com/cli/latest/userguide/getting-started-install.html) recent enough for ECS Express Mode. `aws ecs create-express-gateway-service help` must work.
- Configured AWS credentials. `aws sts get-caller-identity` must succeed. The AWS principal needs permission to create and pass IAM roles and manage ECS Express Mode, ECR, RDS, Secrets Manager, EC2 networking, and CloudWatch logs.
- A default VPC in the target region with subnets in at least two Availability Zones. Find the VPC with `aws ec2 describe-vpcs --filters Name=is-default,Values=true --query 'Vpcs[0].VpcId' --output text --region <region>`. Then run `aws ec2 describe-subnets --filters Name=vpc-id,Values=<default-vpc-id> --query 'Subnets[].AvailabilityZone' --output text --region <region>` and confirm that it returns at least two distinct zones.
- Docker running. The script builds the image locally and pushes it to ECR.
...
Provisions an ECR repo, a private RDS PostgreSQL 17 instance, and Secrets Manager secrets, then makes one `aws ecs create-express-gateway-service` call. That call brings the Fargate service, an ALB with HTTPS, security groups, autoscaling, CloudWatch logs and alarms, and a public URL (`https://ag-<id>.ecs.<region>.on.aws`, generated per service). The script pins scaling to a single always-on task and sets `AGENTOS_URL` to the generated URL so scheduled jobs reach the platform. It also generates `MCP_CONNECT_SECRET` into `.env.production` when it's missing, so chat apps can connect over OAuth from the first deploy.

The first run takes 30-45 minutes end to end. Certificate and DNS provisioning takes most of that time, and the script waits until the gateway answers before declaring success. Redeploys take minutes. Region comes from `AWS_REGION` (default `us-east-1`).

<Note>The template has been validated with one ECS task. AgentOS claims due schedules through PostgreSQL. Simultaneous schedule registration and long-lived MCP streams still need validation before you scale the service.</Note>
...
curl -o /dev/null -w '%{http_code}\n' "https://<service-url>/agents"  # 401

aws logs tail /ecs/agent-os --region <region> --follow
```

The app finishes rolling out behind the gateway; first boot pulls the image and waits for the database. `/health` and `/docs` remain public. Protected AgentOS routes such as `/agents` require a valid token.
  </Step>
</Steps>
