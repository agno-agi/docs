...
Choose Modal when your Python team wants to run AgentOS as one always-warm Modal container. The [agentos-modal](https://github.com/agno-agi/agentos-modal) template uses Docker locally and connects production to a provisioned Neon database or an existing PostgreSQL database with pgvector.
...
Coding agents can use the repository skills, live AgentOS API, evals, traces in PostgreSQL, and Modal logs to inspect and improve the platform.
...
<Note>The current script checks Neon authentication even when you supply an external database. External database deployments still need `neonctl auth` until that preflight is conditional.</Note>
...
When `DB_HOST` and `DB_PASS` are set, the script reuses the external database. Supply `DB_HOST`, `DB_USER`, `DB_PASS`, and `DB_DATABASE`; `DB_PORT` defaults to `5432`. The database must support pgvector and TLS. When the host or password is missing, the script creates a Neon Postgres project and saves its connection details. The script writes your config into the `agentos-secrets` Modal secret and deploys `modal_app.py` as one always-warm container. It then sets `AGENTOS_URL` to the stable `https://<workspace>--agentos.modal.run` URL and generates `MCP_CONNECT_SECRET` when your env file does not have one.
  </Step>
...
curl -o /dev/null -w '%{http_code}\n' "https://<workspace>--agentos.modal.run/health"  # 200
curl -o /dev/null -w '%{http_code}\n' "https://<workspace>--agentos.modal.run/agents"  # 401
```
