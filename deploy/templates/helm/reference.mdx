...
`env-sync.sh` syncs only the connection and secret keys, so a new provider key like `ANTHROPIC_API_KEY` never reaches the cluster that way. Deliver it via `extraEnv` and `helm upgrade`.
  </Accordion>
