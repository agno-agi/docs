...
`env-sync.sh` uses a fixed allowlist that includes `RUNTIME_ENV`, `AGENTOS_URL`, `JWT_JWKS_FILE`, the template's supported secrets, and `DB_PASS`. It does not sync a new provider key such as `ANTHROPIC_API_KEY`. Deliver that key via `extraEnv` and `helm upgrade`.
  </Accordion>
