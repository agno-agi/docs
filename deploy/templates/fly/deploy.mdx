...
Choose Fly.io when you want an always-on AgentOS Machine and PostgreSQL on Fly's private network. The [agentos-fly](https://github.com/agno-agi/agentos-fly) template uses Docker locally and provisions both Fly apps for production.
...
Deploys use `fly deploy --ha=false` because the Fly default creates two machines and doubles the application cost. The template has been validated with one always-on machine. Default sizing is `shared-cpu-2x` with 4 GB (~$21/mo) plus a small Postgres machine (~$4/mo). For dedicated cores, switch the size to `performance-2x` (~$62/mo) in `fly.toml`.

<Note>Fly's stock `postgres-flex` image supports sessions and memory. Knowledge bases require a pgvector-enabled derivative. Set `FLY_PG_IMAGE` before running `up.sh`. The script warns when the stock image is selected. See [Enable pgvector](/deploy/templates/fly/reference#customize).</Note>
  </Step>
...
```bash
curl -o /dev/null -w '%{http_code}\n' "https://<app>.fly.dev/health"  # 200
curl -o /dev/null -w '%{http_code}\n' "https://<app>.fly.dev/agents"  # 401

fly logs
```

The app name comes from `fly.toml`, so no `--app` flag is needed. `/health` and `/docs` remain public. Protected AgentOS routes such as `/agents` require a valid token.
  </Step>
</Steps>
