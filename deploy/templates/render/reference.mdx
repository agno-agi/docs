...
<Warning>
  The scripts search every workspace visible to `RENDER_API_KEY` and select the first exact `agent-os` service and `agentos-db` database. Before `env-sync.sh`, `redeploy.sh`, or `down.sh`, confirm that each name has exactly one visible match and that both returned `ownerId` values identify the intended workspace.
</Warning>
