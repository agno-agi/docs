...
Start with [Deploy AgentOS on Railway](/deploy/templates/railway/deploy) for local setup and the first production deployment.
...
| `RUNTIME_ENV` | No | `prd` | `dev` sets `authorization=False`, which disables AgentOS scope enforcement. Configured JWT environment variables continue to enable JWT validation. Compose sets `dev` locally; keep `prd` on Railway. |
