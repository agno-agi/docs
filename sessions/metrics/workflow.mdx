...
| `executor_type` | `"agent"` or `"team"`. Other step types (functions, parallel blocks, nested workflows) are excluded from aggregated workflow metrics. |
| `executor_name` | Name of the executor. |
| `metrics` | `RunMetrics` with tokens, duration, and model information. See [Metrics reference](/reference/run/metrics). |

Session metrics aggregate token usage and cost from step metrics across all runs in the session. They do not include timing.
...
- [Cookbook](https://github.com/agno-agi/agno/tree/v2.7.4/cookbook/04_workflows/)
