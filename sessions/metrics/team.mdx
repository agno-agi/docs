...
- **Per member run**: Each team member run has its own `RunMetrics` on `run_response.member_responses`. Set `store_member_responses=True` to persist member responses with the session in the database.
...
| Per member run | `RunMetrics` | `member_response.metrics` (set `store_member_responses=True` to persist) |
...
- [Cookbook](https://github.com/agno-agi/agno/tree/v2.7.4/cookbook/03_teams/22_metrics/01_team_metrics.py)
