...
ECS Fargate runs the container. RDS PostgreSQL stores data. An Application Load Balancer handles HTTP and HTTPS. Secrets Manager stores credentials.
...
<Warning>
[HTTPS](/deploy/templates/aws/go-live/https) is required to connect to os.agno.com. You'll set this up after deployment.
</Warning>
...
<CardGroup cols={3}>
  <Card title="Configure Settings" icon="gear" href="/deploy/templates/aws/configure/settings">
    Region, subnets, and ECR
  </Card>
  <Card title="Set Up Secrets" icon="key" href="/deploy/templates/aws/configure/secrets">
