...
# Agent 2: Research specialist with its own database
research_agent = Agent(
    name="Research Agent",
    model=OpenAIResponses(id="gpt-5.2"),
    tools=[HackerNewsTools()],
    instructions="You are a research agent. Answer questions concisely.",
    markdown=True,
...
research_agent.run("Latest AI news on HackerNews")
...
<Tip>
  **Use ClickHouse for high-volume tracing.** Traces are an append-heavy, OLAP workload: billions of rows, time-bucketed aggregates, low-cardinality filters. [ClickHouse](/database/providers/clickhouse/overview) is purpose-built for that shape. Pair it with a row-store (Postgres, MySQL, MongoDB) for sessions and memory; route traces to ClickHouse.
  ...
  ```bash
  uv pip install -U clickhouse-connect psycopg
  ```
