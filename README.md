...
4. Delete the `reference-api/openapi.yaml` file.
5. Run `npx @mintlify/scraping@latest openapi-file reference-api/openapi.json -o reference-api/schema` to generate the new API reference
6. Install `swagger-cli` if you don't have it. `npm install -g swagger-cli`. Then create the new `openapi.yaml` file. 
