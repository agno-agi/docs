...
1. Upload the file to your preferred storage service and keep the URL/ID in `metadata`
...
`store_history_messages=False` keeps the runtime behavior (history still reaches the LLM) but scrubs the historic messages before persistence. Only the current run is written to the database.
...
Remember: fetching history later (`get_chat_history`) will only return what you chose to persist. If you still need full transcripts elsewhere, consider exporting them to your own storage service before the run finishes.
