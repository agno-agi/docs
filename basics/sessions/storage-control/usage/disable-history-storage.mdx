...
The `store_history_messages` flag lets you control this behavior. When set to `False`, historical messages are still used during execution (so your agent or team has memory) but they are not persisted when saving the run to the database.
...
- ✅ Your agent/team still has memory during the conversation
- ✅ History is added to context as configured by `add_history_to_context` and `num_history_runs`
- ✅ Only the database storage is reduced
- ⚠️ When retrieving past runs, historical messages won't be included

## When to Use This

Use `store_history_messages=False` when:

- **Cost optimization**: You want to reduce database storage costs for high-volume systems
- **Privacy compliance**: Historical messages contain sensitive data that shouldn't be persisted long-term
- **Debug focused**: You only care about storing the current interaction, not the full conversation history
- **Performance**: You want faster database writes by reducing data volume
