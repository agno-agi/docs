...
workflow = Workflow(
    ...
    db=db,  # Enable persistence
    session_id="user_123",  # Same ID = same workflow session
    steps=[
        Step(name="Research", agent=Agent(model=OpenAIChat(id="gpt-4o-mini"))),
    ],
)
...
db = PostgresDb(
    db_url="postgresql+psycopg://user:pass@localhost:5432/dbname",
    session_table="team_sessions"  # Optional: custom table name
)
```
...
db = PostgresDb(
    db_url="postgresql+psycopg://user:pass@localhost:5432/dbname",
    session_table="workflow_sessions"  # Optional: custom table name
)
```
...
<Note>
In-memory databases work the same for agents, teams, and workflows.
</Note>
