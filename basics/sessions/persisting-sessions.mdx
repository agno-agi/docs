...
The same constructor arguments work for `Team` and `Workflow`. For a full runnable script (including environment setup) see `/examples/basics/agent/session/01_persistent_session.mdx` and `/examples/basics/teams/session/01_persistent_session.mdx`.
