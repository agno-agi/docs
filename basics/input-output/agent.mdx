...
description: Control agent inputs and outputs with type-safe Pydantic models for reliable, production-ready systems.
keywords: [agent input, agent output, structured output, structured input, pydantic, type safety, validation, agent schema]
---

`Agents` support various forms of input and output, from simple string-based interactions to structured data validation using Pydantic models.
...
- [Images / Audio / Video / Files as Input](/examples/basics/multimodal)
- [List as Input](/examples/basics/agent/input-and-output/input-as-list)
</Note>
...
Agents support structured input using Pydantic models or `TypedDict`. Pass them to the `input` parameter in `Agent.run()` or `Agent.print_response()`.
