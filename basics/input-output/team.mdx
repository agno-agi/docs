...
sidebarTitle: Input & Output with Teams
description: Control team inputs and outputs with type-safe Pydantic models for reliable, production-ready systems.
keywords: [team input, team output, structured output, structured input, pydantic, type safety, validation, team schema]
---

`Teams` support various forms of input and output, from simple string-based interactions to structured data validation using Pydantic models.

Here's an example using string-based input and output:
...
Structured output lets you generate type-safe data using Pydantic models, ensuring consistent and predictable responses for production systems.

Use this feature to extract information, classify data, or generate structured content. Structured outputs integrate seamlessly with tools, knowledge bases, and other Agno features.
...
Teams support structured input using Pydantic models. Pass them to the `input` parameter in `Team.run()` or `Team.print_response()`.
...
For detailed examples and patterns of typesafe implementations, see the [Input & Output with Agents](/basics/input-output/agent#typesafe-agents), which demonstrates the same concepts that apply to Teams.
