...
- Token counting is still in Beta. We do our best to provide an estimate but we do not claim it be be 100% accurate. Please do not use this token count for calculating costs.
- For some providers like Claude we are able to call the exact endpoint and get an
  exact token count. This is not supported for all providers yet.
