...
```mermaid
graph LR
    A[User Input] --> B[Team Leader]
    B --> C{Delegate to Member}
    C -->|Synthesizes Input| D[Member Agent]
    D -->|Direct Response| E[User]
```
...
graph LR
    A[User Input] --> B[Team Leader]
    B --> C{Delegate to Member}
    C -->|Send Raw User Input| D[Member Agent]
    D -->|Returns Response| E[Team Leader]
    E -->|Synthesizes Response| F[User]
```
...
graph LR
    A[User Input] --> B[Team Leader]
    B -->|Delegates| C[Member 1]
    B -->|Delegates| D[Member 2]
    B -->|Delegates| E[Member 3]
    C -->|Response| F[Team Leader]
    D -->|Response| F
    E -->|Response| F
    F -->|Synthesizes| G[User]
```
