...
news_agent = Agent(
    ...
    role="Get the latest tech news from HackerNews",
...
response = team.run(input="What are the trending AI stories and how are AI stocks doing?")
...
Run the team asynchronously using `Team.arun()`. Members will run concurrently if the team leader delegates to multiple members in one request.

</Tip>

<Tip>

See [Input & Output](/basics/input-output/overview) for structured input and output with teams.
...
stream: Iterator[TeamRunOutputEvent] = team.run("What are the top AI stories?", stream=True)
for chunk in stream:
...
response_stream = team.run("What are the top stories?", stream=True, stream_events=True)
...
See [Sessions](/basics/sessions/overview) for details.
...
See [Multimodal](/features/multimodal/overview) for details.
...
agent = Agent(name="Analyst", model=OpenAIResponses(id="gpt-5.2"))
team = Team(members=[agent], model=OpenAIResponses(id="gpt-5.2"))
team.run("Analyze the market", output_schema=DetailedReport)
```

See [Input & Output](/basics/input-output/overview) for details.
...
- [Team Cookbook](https://github.com/agno-agi/agno/tree/main/cookbook/04_teams/README.md)
