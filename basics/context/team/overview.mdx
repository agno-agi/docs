...
The member information is automatically injected into the system message. This includes the member ID, name, and role.
You can optionally include member tools by setting `add_member_tools_to_context` to `True`, which adds the tools available to each member in the system message.
