...
Similar to [workflow history for steps](/basics/chat-history/workflow), the `WorkflowAgent` has access to the full history of workflow runs for the current session.
...
Explore the different examples in [Conversational Workflows Examples](/examples/basics/workflows/workflows-advanced-concepts/conversational-workflows/conversational-workflow-with-conditional-step) section for more details.