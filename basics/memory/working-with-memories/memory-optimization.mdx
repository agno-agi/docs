...
With agentic memory enabled, this becomes even more expensive since all memories are loaded into context for each memory operation.

## The Solution: Memory Optimization

Memory optimization uses LLM-based summarization to combine multiple memories into fewer, more efficient memories. The default **Summarize** strategy combines all memories into a single comprehensive summary, dramatically reducing token usage while preserving all factual information.
...
<Steps>
    ...
    <Step title="Track Savings">
        Token counts before and after optimization are tracked to measure impact.
    </Step>
</Steps>
...
# After some conversations, optimize memories for a user
...
preview = agent.memory_manager.optimize_memories(
    user_id="user_123",
    strategy=MemoryOptimizationStrategyType.SUMMARIZE,
...
if preview:
    print(f"Original memories: {len(agent.memory_manager.get_user_memories('user_123'))}")
    print(f"Optimized to: {len(preview)} memory")
    print(f"Content: {preview[0].memory}")
```

### Using a Custom Model for Optimization

You can use a cheaper model specifically for optimization to reduce costs:

```python
from agno.memory import MemoryManager
from agno.models.openai import OpenAIChat

# Use a cheaper model for optimization
memory_manager = MemoryManager(
    db=db,
    model=OpenAIChat(id="gpt-4o-mini"),  # Cheaper model for optimization
)

# Optimize with the cheaper model
...
You can also optimize memories using a standalone `MemoryManager`:
...
# Setup database and memory manager
db = SqliteDb(db_file="agno.db")
memory_manager = MemoryManager(
    db=db,
...
print(f"Optimized memories: {len(optimized)}")
...
You can measure token savings by comparing before and after:
...
1. **Preview before applying:** Always test with `apply=False` first
2. **Use cheaper models:** Use `gpt-4o-mini` or similar for optimization
3. **Optimize periodically:** Set up scheduled jobs for long-running apps
4. **Monitor token savings:** Track reduction percentages to measure impact
5. **Backup before optimization:** Consider backing up memories before applying
...
See the [Memory Optimization Example](/examples/basics/memory/memory-optimization) for a complete working example that demonstrates:
