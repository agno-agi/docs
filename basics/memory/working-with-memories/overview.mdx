...
As users accumulate memories over time, token costs can grow significantly. Memory optimization helps reduce these costs by combining multiple memories into fewer, more efficient memories while preserving all key information.
...
See the [Memory Optimization](/basics/memory/working-with-memories/usage/memory-optimization) guide for detailed usage and best practices.
