...
If after a tool call we need to provide feedback to the model to change its behavior or exit the tool call loop, we can raise one of the following exceptions:

- `RetryAgentRun`: Use this exception when you want to provide instructions to the model for how to change its behavior. The exception message will be passed to the model as a tool call error, allowing the model to retry or adjust its approach in the next iteration of the LLM loop. **Note:** This does not retry the full agent run—it only provides feedback to the model within the current run.

- `StopAgentRun`: Use this exception when you want to exit the tool call loop. When raised from a tool function, the agent exits the tool call loop, and the run status is set to `COMPLETED`. All session state, messages, tool calls, and tool results up to that point are stored in the database. **Note:** This does not cancel the agent run—it completes the run after exiting the tool call loop.

## Example: Using RetryAgentRun
