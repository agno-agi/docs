...
## Accessing Agent or Team in Tools

Agno automatically injects special built-in parameters into your tool functions. You can access the `Agent` or `Team` instance directly by adding `agent` or `team` as a parameter, giving you access to properties like `dependencies`, `session_state`, and more.

```python
from agno.agent import Agent

def get_data(agent: Agent) -> str:
    """Fetch data using agent dependencies."""
    limit = agent.dependencies.get("limit", 10) if agent.dependencies else 10
