...
For toolkits that need to perform async operations (like HTTP requests, database queries, or browser automation), you can provide both sync and async variants of your tools. The framework automatically uses the appropriate version based on the execution context:
- `agent.run()` / `agent.print_response()` → uses sync tools
- `agent.arun()` / `agent.aprint_response()` → uses async tools

To create an async-capable toolkit, pass async tool variants using the `async_tools` parameter:
...
- For async toolkits, keep the sync and async method signatures identical (same parameters) so the LLM-generated tool calls work with both variants.
</Tip>
