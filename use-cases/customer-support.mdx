...
A support system needs persistent conversation history, access to approved information, and clear boundaries for customer-impacting actions.
...
The team can consult both specialists for this request. Reuse the `session_id` for later messages on the same ticket and the `user_id` for the same customer. This example routes requests and stores session history. Add confirmation-required tools before permitting account changes or refunds.
