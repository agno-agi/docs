...
Each `(prompt, A, B, winner)` row is a raw pairwise comparison. Convert the winner into the `chosen` and `rejected` fields or other schema required by your reward-model or DPO trainer. Ties usually need separate handling.
...
class Preference(BaseModel):
    ...
    rationale: str = Field(..., description="Why this comparison received its verdict")
```

## Compare against a rubric

When preference should follow explicit criteria, put the rubric in the instructions and keep the output constrained to `A`, `B`, or `tie`.
...
| Criteria-driven preference | Rubric in instructions, closed-set output |
...
- [Pairwise preference cookbook](https://github.com/agno-agi/agno/tree/v2.7.4/cookbook/data_labeling/_05_text_pairwise_preference)
