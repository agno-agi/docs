...
This occurs because JWT verification is preferred over security key. Learn more about [security key deprecation](https://docs.agno.com/agent-os/security/overview#security-key-authentication-deprecated).
...
<AccordionGroup>
  ...
  <Accordion title="Option 2: Switch to Authorization (Preferred)" defaultOpen={true}>
    ...
    3. **Ensure JWT verification is enabled** on the AgentOS Control Plane and set the verification key. [More info](https://docs.agno.com/agent-os/security/overview#switching-to-rbac)
