...
### For Older Versions (Before v2.3.12)


If you are running an AgentOS version older than **v2.3.12**, the platform only supports security key authentication. You should:

1. **Disable JWT verification** in from the AgentOS platform
2. Continue using security key authentication only

### For Newer Versions <Badge icon="code-branch" color="orange"><Tooltip tip="Introduced in v2.3.12" cta="View release notes" href="https://github.com/agno-agi/agno/releases/tag/v2.3.12">v2.3.12</Tooltip></Badge> and Later

If you are running AgentOS version **v2.3.12** or newer, we recommend:

1. **Use JWT verification** as the primary authentication method
2. **Disable security key authentication** in your configuration
3. Switch on JWT verification. [More info](https://docs.agno.com/agent-os/security/overview)
