---
title: Memory with PostgreSQL
description: "Store user memories in PostgreSQL by passing PostgresDb as the agent's database."
---

Store and recall user memories with an agent backed by PostgreSQL.

## Code

```python mem-postgres-memory.py
from agno.agent import Agent
from agno.db.postgres import PostgresDb

# Setup Postgres
db_url = "postgresql+psycopg://ai:ai@localhost:5532/ai"
db = PostgresDb(db_url=db_url)

agent = Agent(
    db=db,
    update_memory_on_run=True,
)

agent.print_response("My name is John Doe and I like to play basketball on the weekends.")
agent.print_response("What do I do on weekends?")
```

## Usage

<Steps>
  <Snippet file="create-venv-step.mdx" />

  <Step title="Set environment variables">
    <CodeGroup>
    ```bash macOS / Linux
    export OPENAI_API_KEY=xxx
    ```

    ```powershell Windows
    $Env:OPENAI_API_KEY="xxx"
    ```
    </CodeGroup>
  </Step>

  <Step title="Install dependencies">
    ```bash
    uv pip install -U agno openai sqlalchemy 'psycopg[binary]'
    ```
  </Step>

  <Snippet file="run-pgvector-step.mdx" />

  <Step title="Run Example">
    <CodeGroup>
    ```bash Mac/Linux
    python mem-postgres-memory.py
    ```

    ```bash Windows
    python mem-postgres-memory.py
    ```
    </CodeGroup>
  </Step>
</Steps>
