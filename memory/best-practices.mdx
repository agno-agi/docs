...
```python
# Recommended: memory processing runs in the background
agent = Agent(
    db=db,
    update_memory_on_run=True  # One background call per run, outside the conversation
)
...
- [Working with memories](/memory/working-with-memories/overview)
