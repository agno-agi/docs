...
description: "Keep conversation history session-scoped and user memory available across a user's sessions."
---

Conversation history is scoped by `session_id`. User memory is scoped by `user_id`, so memories remain available across sessions for the same user.
