...
description: "Build and run an agent that inventories and organizes a folder."
...
# Serve via AgentOS with streaming, session APIs, and API endpoints
...
- Session-scoped conversation history. Add JWT authorization and opt-in user isolation for multi-user deployments.
...
<Tip>
Session records stay in your local SQLite database. The agent sends prompts and workspace tool results to OpenAI because it uses `openai:gpt-5.5`.
</Tip>
