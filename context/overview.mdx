...
The general approach is to place repetitive content and common instructions in a stable request prefix that the provider can reuse across requests.
Prompt caching reduces the cost and latency of processing that repeated prefix. The complete prompt is still included in each request.
...
- [Anthropic prompt caching](https://platform.claude.com/docs/en/build-with-claude/prompt-caching) and an [Agno example](/models/providers/native/anthropic/usage/prompt-caching)
