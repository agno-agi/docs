...
description: "Seeds a local markdown wiki and uses WikiContextProvider with FileSystemBackend so the agent writes and reads back a runbook with no auth or network."
...
Full source: [cookbook/12_context/14_wiki_filesystem.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/12_context/14_wiki_filesystem.py)
