...
<Warning>
  The pinned source uses Google Drive's default service-account instructions. Because this page sets `GOOGLE_DELEGATED_USER` for Gmail, Drive also impersonates that Workspace user. The rendered fence intentionally replaces the Drive instructions with delegated-user guidance.
</Warning>
