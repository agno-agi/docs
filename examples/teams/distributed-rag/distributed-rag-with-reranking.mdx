...
<Warning>
  This example passes `url=` to `insert_many()` and `ainsert_many()`. Agno v2.7.4 reads the `urls` argument, so these calls insert no documents. The code fence remains source-exact. Apply the replacement below before running.
</Warning>
...
<Steps>
  ...
  <Step title="Fix the batch insert arguments">
...
Full source: [cookbook/03_teams/15_distributed_rag/03_distributed_rag_with_reranking.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/03_teams/15_distributed_rag/03_distributed_rag_with_reranking.py)
