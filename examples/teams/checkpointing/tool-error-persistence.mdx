...
description: "Two error scenarios for a Team: a caught tool exception (conversation preserved) versus a model-call failure that escapes the loop (recovered via a flush helper), then /continue retries the failed run."
