...
This example demonstrates **Workflows** using parallel steps with custom function 
executors and streaming on AgentOS. The agents and teams running inside the custom function step in `Parallel` will also stream their results in real time in AgentOS.
