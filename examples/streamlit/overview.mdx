---
title: Streamlit Apps
sidebarTitle: Overview
description: 3 production-ready applications with Streamlit UI for RAG, SQL, and search.
---

Full-stack AI applications - not toy demos. Each app solves a real problem with proper architecture.

## Examples

<CardGroup cols={3}>
  <Card title="Agentic RAG" icon="database" href="/examples/streamlit/agentic-rag">
    Document Q&A with knowledge base, citations, and conversation memory.
  </Card>
  <Card title="Text to SQL" icon="table" href="/examples/streamlit/text-to-sql">
    Natural language database queries with dynamic few-shot learning.
  </Card>
  <Card title="Answer Engine" icon="magnifying-glass" href="/examples/streamlit/answer-engine">
    Perplexity-style search with multi-source synthesis.
  </Card>
</CardGroup>

## Running the Apps

```bash
git clone https://github.com/agno-agi/agno.git
cd agno

python3 -m venv .venv
source .venv/bin/activate

# Install for specific app
uv pip install -r cookbook/02_examples/streamlit_apps/agentic_rag/requirements.txt

# Run
streamlit run cookbook/02_examples/streamlit_apps/agentic_rag/app.py
```

Open `localhost:8501` to use the application.

## Next

- [Agent Examples](/examples/agents/overview) - Standalone agent patterns
- [Team Examples](/examples/teams/overview) - Multi-agent coordination
- [Learning Examples](/examples/learning/overview) - Agents that improve over time
