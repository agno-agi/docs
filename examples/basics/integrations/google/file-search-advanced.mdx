...
tech_operation = model.upload_to_file_search_store(
    file_path=Path("documents/technical_manual.txt"),
...
marketing_operation = model.upload_to_file_search_store(
    file_path=Path("documents/product_brochure.txt"),
...
model.delete_file_search_store(technical_store.name, force=True)
print(f"✓ Deleted {technical_store.name}")

model.delete_file_search_store(marketing_store.name, force=True)
