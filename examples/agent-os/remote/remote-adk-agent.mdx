...
description: "Call a Google ADK facts agent over A2A JSON-RPC with RemoteAgent, covering single-shot, streaming, and agent-card lookup."
source: cookbook/05_agent_os/remote/04_remote_adk_agent.py
---

Example demonstrating how to connect to a remote Google ADK agent.

<Warning>
  This example's source docstring names a removed cookbook path: `cookbook/06_agent_os/remote/adk_server.py`. Follow the generated Run step below.
</Warning>

```python remote_adk_agent.py
"""
Example demonstrating how to connect to a remote Google ADK agent.
...
Full source: [cookbook/05_agent_os/remote/04_remote_adk_agent.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/05_agent_os/remote/04_remote_adk_agent.py)
