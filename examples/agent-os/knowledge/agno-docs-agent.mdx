...
description: "Serve a docs-answering agent with PgVector hybrid search knowledge over the Agno docs in AgentOS."
source: cookbook/05_agent_os/knowledge/agno_docs_agent.py
---

Serve an Agno docs knowledge agent through AgentOS. Its pinned media instruction branches require migration before use.

<Warning>
  The pinned source tells the agent to call `text_to_speech` and `create_image`, but registers neither tool.
</Warning>
...
<Steps>
  ...
  <Step title="Remove the media instructions">
...
Full source: [cookbook/05_agent_os/knowledge/agno_docs_agent.py](https://github.com/agno-agi/agno/blob/main/cookbook/05_agent_os/knowledge/agno_docs_agent.py)
