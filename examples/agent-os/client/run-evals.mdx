...
description: "Trigger accuracy, performance and reliability evals (with expected tool calls and argument matching) against a remote agent, then list eval runs and fetch one run's details."
...
Full source: [cookbook/05_agent_os/client/08_run_evals.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/05_agent_os/client/08_run_evals.py)
