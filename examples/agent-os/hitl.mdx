...
```python cookbook/agent_os/hitl_confirmation.py
"""
AgentOS with Human-in-the-Loop

This example shows how to implement HITL in AgentOS. When an agent needs to execute
a tool that requires confirmation, the run pauses and waits for user approval.

Run: python cookbook/agent_os/hitl_confirmation.py

Prerequisites:
pip install -U fastapi uvicorn sqlalchemy pgvector psycopg openai agno
"""
...
# The response will have status: "PAUSED" and requires_confirmation: true
...
<CardGroup cols={2}>
  <Card
    title="HITL Feature Guide"
    icon="hand"
    href="/agent-os/features/human-in-the-loop"
  >
    Complete guide to HITL in AgentOS
  </Card>
  <Card
    title="HITL Concepts"
    icon="book"
