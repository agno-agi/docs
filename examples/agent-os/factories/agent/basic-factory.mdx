...
description: "Serve an AgentFactory that builds a fresh per-tenant Agent from RequestContext.user_id on every request, alongside a static prototype agent, backed by PostgresDb."
...
Full source: [cookbook/05_agent_os/factories/agent/01_basic_factory.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/05_agent_os/factories/agent/01_basic_factory.py)
