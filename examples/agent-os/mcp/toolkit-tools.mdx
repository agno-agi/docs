---
title: "Serve a toolkit as MCP tools"
description: "Pass a Toolkit to MCPConfig.tools and AgentOS flattens it into one MCP tool per method, the way an agent takes it apart."
source: cookbook/05_agent_os/14_mcp/toolkit_tools.py
---

Pass a Toolkit to MCPConfig.tools and AgentOS flattens it into one MCP tool per method, the way an agent takes it apart. MemoryTools becomes get_memories, add_memory, update_memory, and delete_memory.

```python toolkit_tools.py
"""
...
<Steps>
  ...
  <Step title="Run the example">
    Save the code above as `toolkit_tools.py`, then run:
    ```bash
    python toolkit_tools.py
    ```
  </Step>
...
Full source: [cookbook/05_agent_os/14_mcp/toolkit_tools.py](https://github.com/agno-agi/agno/blob/v3.0.4/cookbook/05_agent_os/14_mcp/toolkit_tools.py)
