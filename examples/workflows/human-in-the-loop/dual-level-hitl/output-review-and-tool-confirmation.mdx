...
description: "Review step output after execution, then confirm the agent's tool call during a streaming workflow."
source: cookbook/04_workflows/08_human_in_the_loop/dual_level_hitl/05_output_review_and_tool_confirmation.py
---

Two HITL levels on a single step - one pre-execution, one post-execution: Pause 1 (executor-level): Agent's tool has requires_confirmation=True -> user confirms the tool call DURING execution Pause 2 (step-level): Step has requires_output_review=True -> AFTER the agent completes, user reviews the output and can approve, reject (retry with feedback), or edit
