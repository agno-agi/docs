...
description: "Validate financial-advice input with a pre-hook, then detect blocked calls by checking for RunStatus.error."
source: cookbook/02_agents/09_hooks/pre_hook_input.py
---
...
<Warning>
  The source catches `InputCheckError` around blocked `Agent.run()` calls. Agno v2.7.4 converts the pre-hook exception into a run output with `RunStatus.error`, so those `except` blocks are bypassed. Check each blocked response's status instead.
</Warning>
...
<Steps>
  ...
  <Step title="Check blocked run statuses">
...
Full source: [cookbook/02_agents/09_hooks/pre_hook_input.py](https://github.com/agno-agi/agno/blob/v2.7.4/cookbook/02_agents/09_hooks/pre_hook_input.py)
