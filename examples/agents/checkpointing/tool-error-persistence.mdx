...
description: "Persist the conversation through tool exceptions and failed model calls, then retry the failed run in place with /continue."
source: cookbook/02_agents/18_checkpointing/02_tool_error_persistence.py
---

Run two failure scenarios to verify that tool exceptions and model errors preserve the conversation for `/continue` retries.
