...
<Note>
For a detailed overview of response caching, see [Response Caching](/concepts/models/cache-response).
</Note>

<Note>
This is different from Anthropic's prompt caching feature. Response caching caches the entire model response, while [prompt caching](/examples/models/anthropic/prompt_caching) caches the system prompt to reduce processing time.
</Note>
...
```python cache_model_response_stream.py
import time
