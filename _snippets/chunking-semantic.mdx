...
| --------- | ---- | ------- | ----------- |
| `embedder` | `Union[str, Embedder, BaseEmbeddings]` | `OpenAIEmbedder` | The embedder to use for generating embeddings. Can be: a string model identifier (e.g., `"text-embedding-3-small"`) that chonkie resolves via its provider registry, any Agno embedder (e.g., `AzureOpenAIEmbedder`, `MistralEmbedder`), or a chonkie `BaseEmbeddings` subclass for custom implementations. |
